...
- 客户端
  - rpc调用的时候，是一个stub，相当于一个存根，然后将代码执行，这里相当于用channel进行封装的，然后塞入到 stub 里面，然后其实调用的就是channel相关的方法，其中channel将其重写了，所以传入的时候，就是直接channel，
  - channel里面封装了相关调用，所以说执行的是对应的channel里面的东西，然后执行相关的函数。
  - 然后执行就可以了，关键是这个stub，
- 服务端
  - 服务端启动的话，就开始start了，会将注册的fd塞入EventLoop里面，然后进行监听


## 阴暗杂点

- 链接顺序：我以前以为cmake链接的时候，链接顺序随便链接的，但是其实链接的顺序也是有区别的，`越基础的库越应该放在前面`，比如说fmt等库，就应该放在前面，否贼链接的时候就会链接失败，并且在动态链接的时候，也应该将这个动态库进行编译，否则可能会导致错误
  - 改成方法：将fmt等库放在前面即可，并且编译一定要在本地机子上编译，然后运行
- 函数调用的死锁问题，`函数重复调用，导致死锁`
  - 这个问题可以使用可重复性的锁，就是可以一直加锁的那个，但是我觉得当项目代码一多，那么这个锁稍不注意，就会出错
    - 最终的解决方案是：干脆不写这个函数了，直接放在上面，虽然啰嗦一下，但是能运行
      - 如果写了这个函数，调用了这个函数，可能会有一下错误，
        - 当函数不小心调用的时候，可能会出现多线程错误
        - 以后维护的时候，如果说这个地方应该加锁，一加，结果寄了，不好调试
        - 还有一种方法是直接写在注释里面，但是感觉不是很清晰
- protobuf链接失败，因为protobuf设置的时候，没有相关config，导致需要find_package进行链接
  - 可以参考我写的[这篇文章](https://zhuanlan.zhihu.com/p/679834990)
  - 调试的时候，如果异常断开，这时候就会处于time_wait状态，导致连接不上
    - 正常来说，不会出现这个情况，因为异常断开的话，是ctrl+c这样断开了，那么就会出现一场，如果是正常退出，不会出现这个异常，
    - 但是如果真的要解决这个问题，是可以使用`setsockopt()`进行端口复用解决
- 队列没有锁，导致多个线程访问buffer文件，因为`.top()`访问没有文件的信息的时候，就会出现UB的行为，这时候就会出错
  - 很简单，直接加锁就行了
- 重复引用调用导致空指针异常
  - 这个问题是：在初始化日志的时候，如果说你在初始化日志的时候调用相关日志的方法，那么你就会导致空指针异常，导致寄了
    - 问题解决差不多，gdb可以运行到错误的地方，然后`backtrace`打出调用栈，发现空指针异常了，（0x00）
    - 使用万能的pritnf打法进行调试，然后查出问题，不要在这里进行打印，直接使用cout输出文件就行了
- 当客户但发送连接的话，用户端接受，结果一直触发可写事件（日志一直刷新）
  - 在结束的时候。。要将事件的可读可写事件取消掉。。。
- 哎，创建日志的时候，记得一定要创建，排查了一上午。。。，FILE*好像并不会自动创建文件的...真逆天啊
- 服务器部署的时候，ip绑定的时候，应该是`0.0.0.0`而不是`服务器的ip地址`，要不然直接报errno = 99 这样的错误
