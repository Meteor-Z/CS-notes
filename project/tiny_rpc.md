...
- protobuf序列化的的规则
  - protobuf序列化的时候将其压缩了，`variant`编码方式,`zigzag`编码方式等，将其压缩，所以说压缩的数据的大小是比较低的，而且压缩的时间很快。
  - 
...
- 怎么提高并发量优化，怎么提高线程池的设计优化，任务过多的时候，线程池如何设计优化？
  ...
  - subReactor可以根据性能，我设置的是根据cpu的线程数进行计算的，不过这个可以写在配置文件中，进行更改
  - 这里用了一个非常小的复杂均衡，就是每次从iothreadgroup取出来一个iothread，都会计算一下这个任务的多少，取出来最小的那个
- 异步日志是怎么设计的
  - 异步日志
- 线程池怎么设计优化
  - 
